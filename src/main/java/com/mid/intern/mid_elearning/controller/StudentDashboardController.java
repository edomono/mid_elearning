...
public class StudentDashboardController {
    ...
    public String courseDetails(@PathVariable("id") Long id, Authentication authentication, Model model) {
        try {
            ...
            // ✅ Ambil semua assignment untuk course ini
    ...
    @PostMapping("/user/assignment/{id}/submit")
    public String submitAssignment(@PathVariable("id") Long id,
        ...
        try {
            ...
            if (assignment.getDueDate() != null && LocalDateTime.now().isAfter(assignment.getDueDate())) {
                return "redirect:/user/assignment/" + id + "?error=The deadline for this assignment has passed.";
            }



            java.util.Optional<com.mid.intern.mid_elearning.model.Submission> existing =
                    submissionService.getSubmissionByAssignmentAndStudent(assignment, user);

            if (existing.isPresent()) {
                submissionService.updateSubmission(existing.get().getId(), user, file, note);
            } else {
                submissionService.submitAssignment(user, assignment, file, note);
            }

            return "redirect:/user/assignment/" + id + "?success";
        } catch (Exception e) {
            return "redirect:/user/assignment/" + id + "?error";
        }
    }
